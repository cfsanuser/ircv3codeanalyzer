...
IRCV3_FEATURES = {
    ...
    "sts": {
        ...
        "patterns": [r"\bSTS\b", r"strict_transport", r"sts_policy",
                     r"sts_persist", r"sts.*security", r"draft/sts",
                     r"draft/strict-transport-security", r"STS.*policy"],
    },
    "invite_notify": {
        "name": "Invite Notify",
        "spec": "ircv3.2",
    ...
    "isupport_tokens": {
        "name": "ISUPPORT Token Awareness",
        "spec": "ircv3.1",
        "patterns": [
            r"\bCASEMAPPING\b", r"\bCHANMODES\b", r"\bPREFIX\b",
            r"\bCHANTYPES\b", r"\bNICKLEN\b", r"\bMAXCHANNELS\b",
            r"\bCHANNELLEN\b", r"\bTOPICLEN\b", r"\bKICKLEN\b",
            r"\bMODES\b", r"\bMAXLIST\b", r"\bMAXBANS\b",
            r"\bEXCEPTS\b", r"\bINVEX\b", r"\bNETWORK\b",
            r"\bSAFELIST\b", r"\bSTATUSMSG\b", r"\bELIST\b",
            r"\bMONITOR\b", r"\bTARGMAX\b", r"\bWATCH\b",
            r"\bWHOX\b", r"\bWHOWAS\b", r"\bNAMESX\b",
            r"\bUHNAMES\b", r"\bFNC\b", r"\bKNOCK\b",
            r"\bCLIENTVER\b", r"\bKEYLEN\b", r"\bUSERLEN\b",
            r"\bHOSTLEN\b", r"\bAWAYLEN\b", r"\bSILENCE\b",
        ],
    },
    "draft_websocket": {
        "name": "WebSocket Transport (draft)",
        "spec": "ircv3.3",
        "patterns": [r"websocket", r"web.?socket", r"draft/websocket",
                     r"ws://", r"wss://", r"irc.*websocket"],
    },
    "draft_oauth": {
        "name": "OAuth (draft)",
        "spec": "ircv3-draft",
        "patterns": [r"oauth", r"OAUTH", r"OAuth", r"draft/oauth",
                     r"oauth.*token", r"bearer.*token", r"authorize.*irc"],
    },
    "draft_account_extban": {
        "name": "Account Extban (draft)",
        "spec": "ircv3.3",
        "patterns": [r"account.?extban", r"account_extban", r"\$a:",
                     r"draft/account-extban", r"extban.*account"],
    },
    "draft_rate_limit": {
        "name": "Rate Limit (draft)",
        "spec": "ircv3.3",
        "patterns": [r"rate.?limit", r"rate_limit", r"draft/rate-limit",
                     r"@rate-limit=", r"\+rate-limit", r"rate.*limit.*irc"],
    },
    "draft_web_push": {
        "name": "Web Push (draft)",
        "spec": "ircv3-draft",
        "patterns": [r"web.?push", r"web_push", r"draft/web-push",
                     r"push.*notif", r"webpush"],
    },
    "draft_message_redaction": {
        "name": "Message Redaction (draft)",
        "spec": "ircv3-draft",
        "patterns": [r"\bREDACT\b", r"message.?redact", r"message_redact",
                     r"draft/message-redaction", r"redact.*message"],
        "commands": ["REDACT"],
    },
    "draft_channel_redirect": {
        "name": "Channel Redirect (draft)",
        "spec": "ircv3-draft",
        "patterns": [r"\bREDIRECT\b", r"channel.?redirect",
                     r"draft/channel-redirect", r"chan.*redirect"],
        "commands": ["REDIRECT"],
    },
    "draft_join_rate_limit": {
        "name": "Join Rate Limit (draft)",
        "spec": "ircv3-draft",
        "patterns": [r"join.?rate.?limit", r"join_rate_limit",
                     r"draft/join-rate-limit", r"join.*throttl",
                     r"@join-rate-limit="],
    },
    "draft_persistent_members": {
        "name": "Persistent Members (draft)",
        "spec": "ircv3-draft",
        "patterns": [r"persistent.?members?", r"persistent_members",
                     r"draft/persistent-members", r"always-on"],
    },
    "draft_subscribe": {
        "name": "Subscribe (draft)",
        "spec": "ircv3-draft",
        "patterns": [r"\bSUBSCRIBE\b", r"subscribe", r"draft/subscribe",
                     r"push.?model", r"channel.*subscri"],
        "commands": ["SUBSCRIBE"],
    },
    "draft_sasl_mech_display": {
        "name": "SASL Mechanism Display (draft)",
        "spec": "ircv3-draft",
        "patterns": [r"sasl.?mech", r"sasl_mech", r"draft/sasl-mech-display",
                     r"sasl.*mechanism.*list", r"mech.*display"],
    },
    "rpl_numerics": {
        "name": "RPL Numeric Parsers",
        "spec": "ircv3.2",
        "patterns": [
            r"RPL_WHOREPLY\b", r"352\b", r"RPL_TOPICWHOTIME\b", r"333\b",
            r"RPL_CREATIONTIME\b", r"329\b", r"ERR_SASLFAIL\b", r"904\b",
            r"905\b", r"RPL_LOGGEDIN\b", r"900\b", r"RPL_LOGGEDOUT\b",
            r"901\b", r"ERR_NICKLOCKED\b", r"902\b", r"RPL_SASLSUCCESS\b",
            r"903\b", r"ERR_SASLTOOLONG\b", r"906\b", r"ERR_SASLABORTED\b",
            r"907\b", r"ERR_SASLALREADY\b", r"908\b", r"RPL_TOPIC\b",
            r"332\b", r"RPL_NAMREPLY\b", r"RPL_ENDOFNAMES\b",
            r"ERR_CANNOTSENDTOCHAN\b", r"404\b",
        ],
    },
    "draft_cap_notify": {
        "name": "CAP Notify (draft)",
        "spec": "ircv3.2",
        "patterns": [r"cap.?notify", r"cap_notify", r"draft/cap-notify",
                     r"CAP.*NEW\b", r"CAP.*DEL\b", r"cap.*new.*notif"],
    },
    "draft_bot_tag": {
        "name": "Bot Tag (draft)",
        "spec": "ircv3-draft",
        "patterns": [r"bot.?tag", r"bot_tag", r"@\+bot=", r"\+draft/bot",
                     r"draft/bot-tag", r"bot.*flag"],
    },
    "draft_checkpoints": {
        "name": "Checkpoints (draft)",
        "spec": "ircv3.3",
        "patterns": [r"\bCHECKPOINT\b", r"checkpoint", r"draft/checkpoints",
                     r"atomic.*cap", r"cap.*checkpoint"],
        "commands": ["CHECKPOINT"],
    },
    "draft_no_ctcp": {
        "name": "No CTCP (draft)",
        "spec": "ircv3-draft",
        "patterns": [r"no.?ctcp", r"no_ctcp", r"draft/no-ctcp",
                     r"disable.*ctcp", r"ctcp.*disable"],
    },
